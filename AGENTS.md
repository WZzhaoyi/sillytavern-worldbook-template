...
entry_types:
  ...
  # 视角条目（互斥组，同时只激活一个）
  # 文件格式：与设定总集相同的【标题】（关键词）格式
  # 每个条目为一种叙事视角模式，互斥组保证同时只激活一个
  ...
  # 示例：
  #   【第一人称·主角】（我心想,我看着,我记得）
  #   以主角第一人称限制视角叙述，紧贴主角内心独白。
  #   仅描述主角能感知的信息，不写主角不在场的场景。
  #
  #   【第三人称·全知】（他/她心想,远处,另一边）
  #   全知视角，可自由切换场景和角色心理。
  #
  # ⚠️ 关键词精确度要求（防止误触发）：
  # - 关键词必须是该视角下叙述文本中「必然出现」的具体词
  #   ✗ 典型错误：「视角」「第一人称」（不会出现在正文中）
  #   ✓ 典型正确：「我心想」「我看着」「我记得」（第一人称必然出现）
  #   ✓ 典型正确：「他/她心想」「远处」「另一边」（全知视角必然出现）
  # - 避免通用高频词：「的」「是」「了」「说」「想」→ 会误触发
  # - 优先使用动词短语或短句（3-6字），而非单字
  # - 每个模式建议 2-4 个关键词，覆盖该视角的典型叙述特征
  # ─────────────────────────────────────────────────────────────────
...
narrator:
  ...
  persona: >
    ...
    【模式切换】
    叙事模式由世界书「pov」互斥组的条目控制。当前激活的条目即为叙事指令，优先级高于本段默认设定。
    - 默认启用世界书中排列在首位的条目（由 default_index 决定）
    - 可在世界书中启用其他模式（如：对话模式、第一人称等）
    - 各模式互斥，同时只能激活一种
    - 填写本段时，请将你选用的默认视角描述同步写入 style_instructions
...
**目标**：生成角色间的关系条目，以及可选的视角切换条目。格式均为 `【标题】（关键词）`。

**关系网输出**：`literature/fanfic/关系网.txt`
**视角输出**：`literature/fanfic/视角切换.txt`（由 `pov.source_file` 配置）

**指令模板**：

```markdown
请生成关系网条目（及视角切换条目，如需要）。

**参考**：章节概述、设定总集、已生成的角色档案

**输出文件**：
- literature/fanfic/关系网.txt
- literature/fanfic/视角切换.txt（如需视角切换）

**关系网格式**：
【{角色A}与{角色B}的{关系主题}】（{角色A},{角色B},{别名}）
{关系描述与动态表现}

**视角切换格式**（同【标题】（关键词）格式，每个条目为一种互斥叙事模式）：
【{视角模式名}】（{关键词1},{关键词2}）
{叙事指令正文，明确视角人称、信息范围、心理描写方式}

**视角关键词要求**：必须是该视角下叙述文本中必然出现的具体词（3-6字动词短语优先），禁止通用高频词（的、是、了、想），每模式 2-4 个；不精确会导致误触发或漏触发。
```
...
> ⚠️ 正文将作为 first_mes，LLM 视其为「作者示范」，决定后续回复的句式、长度和场景基调。目标 **500-900字**，过短锚定不足，过长拖累节奏。视角和叙事模式由世界书「pov」互斥组控制，剧本应声明适配的模式。
...
- 指定适配的视角模式（参见 literature/fanfic/视角切换.txt 中的条目），并在生成的正文中自然包含对应条目的关键词以触发世界书激活
